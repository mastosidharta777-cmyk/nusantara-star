    ...
    <main className="min-h-screen bg-[#f5f3ee] text-[#171713]">
      <div className="mx-auto max-w-[1440px] px-5 py-8 md:px-10 md:py-10">
        <header className="flex flex-col gap-5 border-b border-black/10 pb-7 md:flex-row md:items-end md:justify-between">
          <div><p className="eyebrow mb-3">Nusantara Star Internal</p><h1 className="text-3xl font-semibold tracking-[-0.03em] md:text-5xl">Dasbor Admin</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-black/60 md:text-base">Brief → pencocokan → ketersediaan → proposal → kesepakatan → booking terjamin.</p></div>
        ...
        <section className="mb-7 border border-black/10 bg-white"><div className="flex items-center justify-between border-b border-black/10 px-5 py-4"><div><p className="text-sm font-semibold">Brief terbaru</p><p className="mt-1 text-xs text-black/45">20 brief terbaru dari Supabase</p></div><span className="text-xs font-semibold text-black/45">{briefs.length} data</span></div>{briefs.length===0?<div className="px-5 py-10 text-sm text-black/50">Belum ada brief tersimpan.</div>:<div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-black/10 bg-black/[0.025] text-xs uppercase tracking-[0.12em] text-black/45"><tr>{["Acara","Tanggal","Kota","Talent","Anggaran","Status","Aksi"].map(x=><th key={x} className="px-5 py-3 font-semibold">{x}</th>)}</tr></thead><tbody>{briefs.map(brief=><tr key={brief.id} className="border-b border-black/5 last:border-0"><td className="px-5 py-4 font-medium">{brief.event_type??"—"}</td><td className="px-5 py-4 text-black/65">{brief.event_date??"—"}</td><td className="px-5 py-4 text-black/65">{brief.city??"—"}</td><td className="px-5 py-4 text-black/65">{brief.talent_category??"—"}</td><td className="px-5 py-4 text-black/65">{money(brief.budget_min)} – {money(brief.budget_max)}</td><td className="px-5 py-4"><span className="border border-black/10 px-2 py-1 text-xs font-semibold uppercase tracking-[0.08em]">{brief.status}</span></td><td className="px-5 py-4"><Link href={`/admin/briefs/${brief.id}`} className="font-semibold underline underline-offset-4">Buka brief</Link></td></tr>)}</tbody></table></div>}</section>
        <section className="border border-black/10 bg-white"><div className="flex flex-col gap-2 border-b border-black/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">Database Talent</p><p className="mt-1 text-xs text-black/45">Status internal, kisaran fee, dan pembaruan kalender</p></div><span className="text-xs font-semibold text-black/45">{talents.length} talent</span></div><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-black/10 bg-black/[0.025] text-xs uppercase tracking-[0.12em] text-black/45"><tr>{["Talent","Kategori","Kota","Kisaran fee","Status internal","Pembaruan ketersediaan"].map(x=><th key={x} className="px-5 py-3 font-semibold">{x}</th>)}</tr></thead><tbody>{talents.map(talent=><tr key={talent.id} className="border-b border-black/5 last:border-0"><td className="px-5 py-4 font-semibold">{talent.name}</td><td className="px-5 py-4 text-black/65">{talent.category}</td><td className="px-5 py-4 text-black/65">{talent.base_city??"—"}</td><td className="whitespace-nowrap px-5 py-4 text-black/65">{money(talent.budget_min)} – {money(talent.budget_max)}</td><td className="px-5 py-4"><span className="border border-black/10 px-2 py-1 text-xs font-semibold uppercase tracking-[0.08em]">{talent.status}</span></td><td className="px-5 py-4"><span className={`inline-flex border px-2 py-1 text-xs font-semibold ${freshnessClass(talent.freshness)}`}>{freshnessLabelId(talent.freshness)}{talent.daysSinceCalendarUpdate!=null?` · ${talent.daysSinceCalendarUpdate} hari`:""}</span></td></tr>)}</tbody></table></div></section>
      </div>
    </main>
  );
}
