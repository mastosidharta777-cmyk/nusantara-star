  ...
  return (
    <main className="min-h-screen bg-[#f5f3ee] text-[#171713]">
      <div className="mx-auto max-w-[1180px] px-5 py-8 md:px-10 md:py-10">
        <Link href="/admin" className="text-sm font-semibold text-black/55 hover:text-black">
          ← Admin Dashboard
        </Link>
        ...
        <header className="mt-6 border-b border-black/10 pb-7">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-3xl font-semibold tracking-[-0.03em] md:text-5xl">{row.event_type ?? "Untitled Brief"}</h1>
              <p className="mt-3 text-sm text-black/55">Brief ID: {row.id}</p>
            </div>
            <span className="w-fit border border-black/10 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em]">
              {row.status}
            </span>
          </div>
        </header>
        ...
        <section className="grid gap-3 py-7 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ...
            ["Budget Min", money(row.budget_min)],
            ["Budget Max", money(row.budget_max)],
          ].map(([label, value]) => (
            <article key={label} className="border border-black/10 bg-white p-4">
        ...
        <section className="border border-black/10 bg-white">
          <div className="border-b border-black/10 px-5 py-4">
            <p className="text-sm font-semibold">Matching Recommendation</p>
            <p className="mt-1 text-xs text-black/45">Calculated from the current Supabase roster. Live confirmation is still required.</p>
          </div>

          {matches.length === 0 ? (
            <div className="px-5 py-10 text-sm text-black/50">Tidak ada kandidat yang memenuhi aturan shortlist saat ini.</div>
          ) : (
              ...
              {matches.map((match, index) => (
                <article key={match.talent.id} className="p-5 md:p-6">
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">#{index + 1} · {tierLabel(match.tier)}</p>
                      <h2 className="mt-2 text-2xl font-semibold tracking-[-0.02em]">{match.talent.name}</h2>
                      <p className="mt-1 text-sm text-black/55">{match.talent.category} · {match.talent.baseCity}</p>
                    </div>
                    <div className="text-left md:text-right">
                      <p className="text-3xl font-semibold">{match.score}</p>
                      <p className="text-xs uppercase tracking-[0.12em] text-black/40">Match Score</p>
                    </div>
                  </div>

                  <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="border border-black/10 p-3 text-sm"><span className="text-black/45">Rate</span><br />{money(match.talent.budgetMin)} – {money(match.talent.budgetMax)}</div>
