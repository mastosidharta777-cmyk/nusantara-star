...
export async function PATCH(request:Request){
 ...
 try{
  ...
  const parts=asset.storage_key.split("/"),file=parts.pop()??"",folder=parts.join("/");const{data:listed,error:le}=await s.storage.from("talent-documents").list(folder,{search:file,limit:20});
  if(le)throw new Error(le.message);
  const uploaded=(listed??[]).find(x=>x.name===file),uploadedSize=Number((uploaded as any)?.metadata?.size??0);
  ...
  const now=new Date().toISOString();const{error:updateError}=await s.from("talent_assets").update({upload_status:"uploaded",uploaded_at:now,buyer_visible:false,updated_at:now}).eq("id",assetId);
  ...
  try{
   ...
   if(de||!downloaded)throw new Error(de?.message??"Rider download failed");
   const sourceText=(await extractRiderText(Buffer.from(await downloaded.arrayBuffer()),asset.mime_type)).replace(/\u0000/g," ").trim();
   if(!sourceText)throw new Error("Dokumen tidak menghasilkan teks yang dapat dibaca");
   const source=submission??talent;if(!source?.name)throw new Error("Nama talent tidak ditemukan");
   const identity=await validateRiderIdentity({sourceText,talentName:source.name,sourceFilename:asset.original_filename});
   if(identity.outcome==="mismatch"){
    await s.from("talent_assets").update({review_status:"rejected",buyer_visible:false,description:`Source mismatch: ${identity.detectedArtist??"artist berbeda"}`,reviewed_at:now,updated_at:now}).eq("id",assetId);
  ...
  return NextResponse.json({error:assetStored?"Rider sudah tersimpan, tetapi pemeriksaan lanjutan gagal.":"Unggah rider gagal",saved:assetStored,detail},{status:500});
 }
}
