...
export async function POST(request: Request) {
  try {
    ...
    if (action === "approve") {
      const { data: availabilityRequest, error: availabilityError } = await supabase.from("availability_requests").select("status").eq("brief_id", briefId).eq("talent_id", talentId).maybeSingle();
      if (availabilityError) throw new Error(availabilityError.message);
      availabilityStatus = availabilityRequest?.status ?? null;
      if (availabilityStatus === "unavailable") return NextResponse.json({ error: "Cannot approve a talent confirmed as unavailable" }, { status: 409 });
    }
    ...
    if (existingMatch?.id) {
      const patch: Record<string, unknown> = { engine_version: existingMatch.engine_version ?? MATCH_ENGINE_VERSION, generated_at: existingMatch.generated_at ?? now };
      if (action === "approve") { patch.admin_approved = true; patch.admin_rejected = false; patch.reviewed_at = now; }
      else if (action === "reject") { patch.admin_approved = false; patch.admin_rejected = true; patch.reviewed_at = now; }
      const { data: updatedMatch, error: updateMatchError } = await supabase.from("match_results").update(patch).eq("id", existingMatch.id).select("id").single();
      ...
      const matchPayload: Record<string, unknown> = {
        brief_id: briefId, talent_id: talentId, score: match.score, tier: match.tier,
        availability_status: match.availabilityStatus, availability_freshness: match.freshness,
        requires_live_confirmation: match.requiresLiveConfirmation, score_breakdown: match.breakdown,
        reasons: match.reasons, engine_version: MATCH_ENGINE_VERSION, generated_at: now,
      };
      if (action === "approve") { matchPayload.admin_approved = true; matchPayload.admin_rejected = false; matchPayload.reviewed_at = now; }
      else if (action === "reject") { matchPayload.admin_approved = false; matchPayload.admin_rejected = true; matchPayload.reviewed_at = now; }
      const { data: insertedMatch, error: insertMatchError } = await supabase.from("match_results").upsert(matchPayload, { onConflict: "brief_id,talent_id" }).select("id").single();
    ...
    if (action === "request_live_confirmation") {
      const { error: requestError } = await supabase.from("availability_requests").upsert({ brief_id: briefId, talent_id: talentId, match_result_id: matchResultId, status: "pending", requested_at: now, responded_at: null }, { onConflict: "brief_id,talent_id" });
      if (requestError) throw new Error(requestError.message);
    }

    const proposedStatus = action === "request_live_confirmation" ? "availability_check" : action === "approve" ? availabilityStatus === "confirmed" ? "shortlisted" : "matching" : "reviewing";
