...
export async function POST(request: Request) {
  try {
    ...
    if (!Number.isInteger(sequenceNo) || sequenceNo < 1 || !Number.isInteger(dueOffsetDays)) return NextResponse.json({ error: "Sequence and offset must be whole numbers" }, { status: 400 });
    if (calculationType === "percentage" && (!Number.isFinite(percentage) || percentage! < 0 || percentage! > 100)) return NextResponse.json({ error: "Percentage must be between 0 and 100" }, { status: 400 });
    if (calculationType === "fixed_amount" && (!Number.isFinite(amount) || amount! < 0)) return NextResponse.json({ error: "Amount must be zero or greater" }, { status: 400 });
