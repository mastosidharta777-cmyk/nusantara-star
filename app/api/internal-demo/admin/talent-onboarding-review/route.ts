...
function missingRiderTable(error:any){return error?.code==="42P01"||error?.code==="PGRST205"||String(error?.message??"").includes("talent_rider_versions")}
function rpcStatus(message:string){return /tidak ditemukan|belum|terlebih dahulu|belum lengkap|belum siap|berubah saat/i.test(message)?409:500}

export async function GET(request:Request){try{if(!ensureAdmin(request))return NextResponse.json({error:"Tidak memiliki akses"},{status:401});const talentId=new URL(request.url).searchParams.get("talentId")??"";if(!talentId)return NextResponse.json({error:"Talent wajib dipilih"},{status:400});const s=getServerClient();const[{data:talent,error:te},{data:submission,error:se},{data:assets,error:ae},{data:rider,error:re}]=await Promise.all([s.from("talents").select("id,name,onboarding_status,public_visible,status").eq("id",talentId).maybeSingle(),s.from("talent_profile_submissions").select("*").eq("talent_id",talentId).maybeSingle(),s.from("talent_assets").select("id,asset_type,provider,storage_key,original_filename,mime_type,size_bytes,title,description,upload_status,review_status,buyer_visible,created_at").eq("talent_id",talentId).order("created_at",{ascending:false}),s.from("talent_rider_versions").select("id,version_no,source_type,source_asset_id,source_filename,normalized_data,missing_questions,answers,normalization_source,status,is_current,updated_at").eq("talent_id",talentId).eq("is_current",true).maybeSingle()]);if(te)throw new Error(te.message);if(!talent)return NextResponse.json({error:"Talent tidak ditemukan"},{status:404});if(se)throw new Error(se.message);if(ae)throw new Error(ae.message);if(re&&!missingRiderTable(re))throw new Error(re.message);const enriched=await Promise.all((assets??[]).map(async asset=>({...asset,preview_url:await buildAdminAssetPreviewUrl(s,asset)})));return NextResponse.json({ok:true,talent,submission,assets:enriched,rider:rider??null,riderMigrationRequired:Boolean(re&&missingRiderTable(re))})}catch(e){return NextResponse.json({error:"Gagal memuat peninjauan pendaftaran talent",detail:e instanceof Error?e.message:String(e)},{status:500})}}
...
export async function PATCH(request:Request){try{if(!ensureAdmin(request))return NextResponse.json({error:"Tidak memiliki akses"},{status:401});const body=await request.json().catch(()=>null);const talentId=typeof body?.talentId==="string"?body.talentId:"";const action=typeof body?.action==="string"?body.action:"";if(!talentId)return NextResponse.json({error:"Talent wajib dipilih"},{status:400});const s=getServerClient();const now=new Date().toISOString();
 if(action==="review_asset"){const assetId=typeof body?.assetId==="string"?body.assetId:"";const decision=body?.decision==="approved"?"approved":body?.decision==="rejected"?"rejected":"";if(!assetId||!decision)return NextResponse.json({error:"Peninjauan media tidak valid"},{status:400});const result=await reviewOnboardingAsset(s,{profileId:talentId,assetId,decision});return NextResponse.json({ok:true,buyerVisible:result.buyerVisible})}
 ...
 return NextResponse.json({error:"Aksi tidak dikenal"},{status:400})}catch(e){return NextResponse.json({error:"Peninjauan pendaftaran talent gagal",detail:e instanceof Error?e.message:String(e)},{status:500})}}
