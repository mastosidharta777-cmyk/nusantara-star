...
export async function GET() {
  ...
  try {
    ...
    const { data: proposal, error: proposalError } = await supabase.from("proposals").insert({ brief_id: briefId, version: 1, status: "selected", expires_at: validUntil, sent_at: now }).select("id").single();
    if (proposalError || !proposal) throw new Error(proposalError?.message ?? "proposal insert failed");
    const { data: item, error: itemError } = await supabase.from("proposal_items").insert({ proposal_id: proposal.id, brief_id: briefId, talent_id: talentId, talent_offer_id: offer.id, buyer_price: 12000000, currency: "IDR", availability_status: "confirmed", offer_valid_until: validUntil, talent_name_snapshot: "Booking Smoke", talent_category_snapshot: "singer" }).select("id").single();
    ...
    const accepted = await supabase.rpc("ns_accept_buyer_terms_v1", { p_booking_id: booking.id });
    ...
    const { data: secured, error: secureError } = await supabase.rpc("ns_secure_booking_v1", { p_booking_id: booking.id });
    if (secureError) throw new Error(secureError.message);
    const secureRow = Array.isArray(secured) ? secured[0] : secured;
    if (secureRow?.booking_status !== "secured") throw new Error("booking not secured");

    const [{ data: finalBooking }, { data: finalBrief }] = await Promise.all([
      supabase.from("bookings").select("status,buyer_terms_accepted_at,buyer_terms_accepted_deal_id,buyer_terms_acceptance_source,financial_security_type,financial_security_status,secured_at").eq("id", booking.id).single(),
      supabase.from("briefs").select("status").eq("id", briefId).single(),
    ]);
    const evidenceBound = finalBooking?.buyer_terms_accepted_deal_id === deal.id && finalBooking?.buyer_terms_acceptance_source === "signed_buyer_link";

    return NextResponse.json({ ok: finalBooking?.status === "secured" && finalBrief?.status === "booked" && evidenceBound, checks: { buyerSelectedNotBooked: true, buyerTermsAccepted: Boolean(finalBooking?.buyer_terms_accepted_at), buyerTermsEvidenceBoundToDeal: evidenceBound, financialSecuritySatisfied: finalBooking?.financial_security_status === "satisfied", noUniversalDeposit: buyerSchedule[0].percentage === 30, secureBookingGate: finalBooking?.status === "secured" }, booking: finalBooking, cleanup: "automatic" });
  } catch (error) {
