...
export default async function TalentPage({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{category?:string;genre?:string;date?:string;city?:string;format?:string}>}){
  ...
  const nationwide=new Set(["indonesia","seluruh indonesia","nasional","nationwide","all indonesia"]);
  const servesCity=(talent:(typeof allTalents)[number],target:string|null)=>{if(!target)return true;const wanted=normalized(target);const base=talent.base_city?normalized(talent.base_city):"";const services=(talent.service_cities??[]).map(normalized);return base===wanted||services.includes(wanted)||services.some(item=>nationwide.has(item))};
  ...
  const locationEligibleTalents=selectedCity?dateEligibleTalents.filter(t=>servesCity(t,selectedCity)):dateEligibleTalents;
