  ...
  return (
    <div className="bg-[#f5f3ee] text-[#171713]">
      <section className="mx-auto max-w-[1100px] px-5 py-12 md:px-10 md:py-16">
        ...
        <p className="mt-5 max-w-2xl text-base leading-7 text-black/60">{isId ? "Setiap opsi di bawah memakai offer yang sudah dikonfirmasi khusus untuk acara ini—bukan kisaran rate profil umum." : "Every option below uses an event-specific confirmed offer, not a generic profile rate range."}</p>
      ...
      <section className="mx-auto max-w-[1100px] px-5 pb-16 md:px-10 md:pb-24">
              ...
              <article key={talent.id} className="overflow-hidden border border-black/10 bg-white">
                ...
                <div className="p-5 md:p-6">
                  ...
                  <div className="mt-5 grid gap-3 border-t border-black/10 pt-4">
                    <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">{isId ? "Fee untuk Event Ini" : "Event-Specific Fee"}</p><p className="mt-2 text-xl font-semibold">{money(talent.buyer_price, locale)}</p></div>
                    ...
                    {talent.payment_terms ? <p className="text-sm"><span className="text-black/45">Payment terms:</span><br />{talent.payment_terms}</p> : null}
        ...
        <div className="mt-8 border border-black/10 bg-white p-5 text-sm leading-6 text-black/55 md:p-6">{selectedTalentId ? isId ? "Talent sudah dipilih. Tim Nusantara Star akan melanjutkan ke finalisasi deal dan booking." : "Talent selected. Nusantara Star will continue to deal finalization and booking." : isId ? "Pemilihan talent belum berarti booking final. Booking baru aman setelah terms dan kondisi finansial terpenuhi." : "Talent selection is not final booking. Booking is secured only after terms and financial conditions are satisfied."}</div>
      </section>
    </div>
  );
}
