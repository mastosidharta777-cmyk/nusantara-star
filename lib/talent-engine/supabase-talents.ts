...
export async function loadEngineTalents(): Promise<{
  ...
  const [{ data: talentData, error: talentError }, { data: availabilityData, error: availabilityError }] = await Promise.all([
    supabase
      .from("talents")
      .select(
        "id,name,category,gender,genres,base_city,service_cities,performance_formats,event_types,audience_tags,budget_min,budget_max,reliability_score,last_calendar_updated_at,status",
      )
  ...
  const talents: EngineTalent[] = (talentData as TalentRow[]).map((row) => ({
    ...
    availability: (availabilityByTalent.get(row.id) ?? []).map((entry) => ({
