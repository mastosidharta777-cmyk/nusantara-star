...
async function photoUrl(supabase: ReturnType<typeof getServerClient>, talentId: string) {
  ...
  const { data, error } = await supabase.storage.from("talent-photos").createSignedUrl(asset.storage_key, 3600);
  return error ? null : data.signedUrl;
}

export async function loadPublicTalents(): Promise<PublicTalentCard[]> {
...
export async function loadPublicTalent(talentId: string): Promise<PublicTalentDetail | null> {
  ...
  return {
    ...
    photo_url: await photoUrl(supabase, talentId),
    videos: (assets ?? []).map((asset) => ({ id: asset.id, title: asset.title, description: asset.description, asset_type: asset.asset_type, url: createR2PresignedUrl("GET", asset.storage_key, 3600) })),
  };
}
