...
export function AdminDealSheetForm({ briefId, talentId, talentName, initialTerms, talentPolicyTemplates }: {
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body?.detail ?? body?.error ?? "Gagal menyimpan detail kesepakatan");
      router.refresh();
    } catch (err) { setError(err instanceof Error ? err.message : "Gagal menyimpan detail kesepakatan"); }
  ...
  return <section className="border border-black/10 bg-white p-5 md:p-6">
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Biaya langsung<input disabled={locked} value={directCosts} onChange={(e) => setDirectCosts(e.target.value)} className="mt-2 w-full border border-black/15 p-3 font-normal" /></label>
      <label className="text-sm font-semibold">Pajak / biaya pembayaran<input disabled={locked} value={taxFees} onChange={(e) => setTaxFees(e.target.value)} className="mt-2 w-full border border-black/15 p-3 font-normal" /></label>
    </div>
    <div className="mt-6"><p className="text-sm font-semibold">Jadwal klien</p><ScheduleEditor party="buyer" rows={buyerSchedule} /></div>
    ...
    <div className="mt-6 grid gap-3 md:grid-cols-2">
      <textarea disabled={locked} value={cancellationTerms} onChange={(e) => setCancellationTerms(e.target.value)} placeholder="Ketentuan pembatalan" className="min-h-24 border border-black/15 p-3 text-sm" />
      <textarea disabled={locked} value={riderNotes} onChange={(e) => setRiderNotes(e.target.value)} placeholder="Catatan rider" className="min-h-24 border border-black/15 p-3 text-sm" />
      <textarea disabled={locked} value={specialConditions} onChange={(e) => setSpecialConditions(e.target.value)} placeholder="Ketentuan khusus" className="min-h-24 border border-black/15 p-3 text-sm" />
      <textarea disabled={locked} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Catatan internal" className="min-h-24 border border-black/15 p-3 text-sm" />
    </div>
    {!locked ? <button type="button" onClick={save} disabled={busy} className="mt-5 border border-black bg-black px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Menyimpan…" : "Simpan Detail Lanjutan"}</button> : null}
