...
type Rider={version_no:number;source_filename?:string|null;missing_questions?:Question[];answers?:Record<string,string>;status:string};
...
export function TalentRiderCompletion({talentId,token}:{talentId:string;token:string}){
  ...
  return <section className="bg-[#f5f3ee] px-5 pb-12 text-[#171713] md:px-10"><div className="mx-auto max-w-3xl border border-black/10 bg-white p-5 md:p-6"><div><p className="text-sm font-semibold">Kelengkapan Rider Utama</p><p className="mt-1 text-xs text-black/50">Lengkapi hanya informasi dasar yang belum ditemukan pada rider Anda.</p></div>{!editable?<p className="mt-4 border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Rider dikunci selama profil ditinjau. Pilih <b>Edit kembali</b> pada profil jika masih ada yang perlu diubah.</p>:null}{questions.length?<><div className="mt-5 space-y-4">{questions.map(q=><label key={q.key} className="block text-sm font-semibold">{q.question}<textarea disabled={!editable} value={answers[q.key]??""} onChange={e=>setAnswers(v=>({...v,[q.key]:e.target.value}))} rows={2} className="mt-2 w-full border border-black/15 px-3 py-3 font-normal disabled:bg-black/5" placeholder="Jawab singkat dan faktual"/></label>)}</div><button disabled={busy||!editable} onClick={submit} className="mt-4 border border-black bg-black px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">{busy?"Menyimpan…":"Simpan & cek ulang"}</button></>:<div className="mt-4 border border-green-700/20 bg-green-50 p-3 text-sm font-semibold text-green-800">✓ Informasi dasar rider sudah lengkap dan siap ditinjau admin.</div>}{message?<p className="mt-3 text-sm font-semibold text-green-700">{message}</p>:null}{error?<p className="mt-3 text-sm font-semibold text-red-700">{error}</p>:null}</div></section>;
}
