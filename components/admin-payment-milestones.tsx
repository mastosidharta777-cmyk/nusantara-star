  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/internal-demo/admin/payment-milestones", {
        ...
        body: JSON.stringify({ bookingId, party, milestoneType, sequenceNo: nextSequence, calculationType,
          ...
          dueBasis, dueOffsetDays: dueBasis === "custom_date" ? 0 : Number(dueOffsetDays),
          customDueDate: dueBasis === "custom_date" ? customDueDate : null,
          refundable: refundability === "yes" ? true : refundability === "no" ? false : null, cancellationNote }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) throw new Error(body?.detail ?? body?.error ?? "Gagal menyimpan jadwal pembayaran");
      router.refresh();
    } catch (err) { setError(err instanceof Error ? err.message : "Gagal menyimpan jadwal pembayaran"); }
    finally { setBusy(false); }
  }

  return <section className="mt-7 border border-black/10 bg-white p-5 md:p-6">
    <div><p className="text-sm font-semibold">Jadwal Pembayaran</p><p className="mt-1 text-xs text-black/45">Jadwal pembayaran klien dan talent dapat berbeda untuk setiap booking. Bagian ini mencatat jadwal, bukan transaksi uang aktual.</p></div>
    ...
    <div className="mt-5 border-t border-black/10 pt-5"><p className="text-sm font-semibold">Tambah Tahapan Pembayaran</p><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <label className="text-sm font-semibold">Pihak<select value={party} onChange={(e)=>setParty(e.target.value as "buyer"|"talent")} className="mt-2 w-full border border-black/15 bg-white px-3 py-3 font-normal"><option value="buyer">Klien</option><option value="talent">Talent</option></select></label>
      <label className="text-sm font-semibold">Jenis pembayaran<select value={milestoneType} onChange={(e)=>setMilestoneType(e.target.value)} className="mt-2 w-full border border-black/15 bg-white px-3 py-3 font-normal"><option value="booking_fee">Biaya booking</option><option value="deposit">Uang muka (DP)</option><option value="balance">Pelunasan</option><option value="full_payment">Pembayaran penuh</option><option value="other">Lainnya</option></select></label>
    ...
    </div><label className="mt-4 block text-sm font-semibold">Ketentuan pembatalan<input value={cancellationNote} onChange={(e)=>setCancellationNote(e.target.value)} placeholder="Contoh: biaya booking hangus jika klien membatalkan" className="mt-2 w-full border border-black/15 px-3 py-3 font-normal" /></label><button onClick={save} disabled={busy} className="mt-4 border border-black bg-black px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">{busy ? "Menyimpan…" : "Tambah Tahapan"}</button>{error ? <p className="mt-3 text-xs font-semibold text-red-700">{error}</p> : null}</div>
  </section>;
}
