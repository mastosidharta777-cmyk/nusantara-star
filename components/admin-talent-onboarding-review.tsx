...
function riderStatus(status?: string) {
  ...
  if (status === "ready_for_admin") return "Siap direview admin";
...
function compactRider(data?: Record<string, unknown>) {
  ...
  if (data.performers_count != null) rows.push(`Performer: ${data.performers_count}`);
  if (data.crew_count != null) rows.push(`Crew: ${data.crew_count}`);
  ...
  const sections: [string,string][] = [["technical_requirements","Technical"],["stage_backline","Stage/backline"],["hospitality","Hospitality"],["transport_requirements","Transport"],["baggage_requirements","Baggage"],["accommodation_requirements","Akomodasi"],["meals_per_diem","Meal/per diem"],["special_requirements","Kebutuhan khusus"]];
...
export function AdminTalentOnboardingReview({ talentId }: { talentId: string }) {
  ...
  return <section className="mb-7 border border-black/10 bg-white p-5 md:p-6">
    ...
    {data?.rider ? <div className="mt-5 border border-black/10 bg-[#f8f7f3] p-4 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><b>Master Rider v{data.rider.version_no}</b><p className="mt-1 text-xs text-black/50">{data.rider.source_filename || (data.rider.source_type === "form_text" ? "Input rider dari form" : "Sumber rider")} · {data.rider.normalization_source === "ai" ? "dinormalisasi backend" : "rules fallback"}</p></div><span className="border border-black/10 bg-white px-2 py-1 text-xs font-semibold">{riderStatus(data.rider.status)}</span></div>
      {riderRows.length ? <ul className="mt-3 space-y-1 text-black/65">{riderRows.map((row)=><li key={row}>• {row}</li>)}</ul> : <p className="mt-3 text-black/50">Belum ada fakta rider terstruktur.</p>}
      {data.rider.missing_questions?.length ? <div className="mt-3 border-t border-black/10 pt-3"><p className="font-semibold">Masih perlu dijawab talent:</p><ul className="mt-2 space-y-1 text-black/60">{data.rider.missing_questions.map((q)=><li key={q.key}>• {q.question}</li>)}</ul></div> : <p className="mt-3 font-semibold text-green-800">✓ Informasi dasar rider lengkap untuk review admin.</p>}
    </div> : data?.riderMigrationRequired ? <p className="mt-5 border border-amber-300 bg-amber-50 p-3 text-sm">Rider Normalization V1 belum diaktifkan di database.</p> : null}
    ...
    <div className="mt-5 space-y-3">{data?.assets?.length ? data.assets.map((asset) => {
      ...
      return <div key={asset.id} className="border-t border-black/10 py-4">
        ...
        {previewed && asset.preview_url ? <div className="mt-4 border border-black/10 bg-[#f5f3ee] p-3">{isImage ? <img src={asset.preview_url} alt={asset.original_filename ?? "Preview foto"} className="max-h-[560px] w-full object-contain" /> : null}{isVideo ? <video src={asset.preview_url} controls preload="metadata" className="max-h-[560px] w-full bg-black" /> : null}{asset.asset_type === "rider_document" && isPdf ? <iframe src={asset.preview_url} title={asset.original_filename ?? "Preview rider"} className="h-[620px] w-full bg-white" /> : null}{asset.asset_type === "rider_document" && !isPdf ? <a href={asset.preview_url} target="_blank" rel="noreferrer" className="text-sm font-semibold underline">Buka dokumen rider di tab baru</a> : null}<p className="mt-2 text-xs text-black/45">Preview privat sementara. Setujui hanya setelah isi benar-benar diperiksa.</p></div> : null}
      </div>;
