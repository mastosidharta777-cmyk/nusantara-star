...
export function AdminTalentOnboardingReview({ talentId }: { talentId: string }) {
  ...
  return <section className="mb-7 border border-black/10 bg-white p-5 md:p-6">
    ...
    <div className="mt-5 space-y-3">{data?.assets?.length ? data.assets.map((asset) => {
      ...
      const isImage = asset.asset_type === "profile_photo"; const isVideo = ["live_performance", "showreel", "event_clip"].includes(asset.asset_type); const isYoutube = asset.provider === "youtube_unlisted"; const isSoundcloud = asset.provider === "soundcloud"; const isInstagram = asset.provider === "instagram"; const isPdf = asset.mime_type === "application/pdf";
