...
function dueLabel(item: TalentPaymentPolicyTemplate) {
  const basis = item.due_basis === "booking_date" ? "booking" : item.due_basis === "event_date" ? "acara" : item.due_basis === "event_completion" ? "acara selesai" : "tagihan";
...
export function AdminTalentCommercialProfile({ talentId, policies }: { talentId: string; policies: TalentPaymentPolicyTemplate[] }) {
  ...
  return <section>
    <div><p className="text-sm font-semibold">Termin pembayaran bawaan</p><p className="mt-1 text-xs text-black/45">Contoh: DP saat booking lalu pelunasan sebelum acara. Ini hanya template internal; ketentuan akhir tetap mengikuti kesepakatan acara.</p></div>
    <div className="mt-5 space-y-2">{policies.length === 0 ? <p className="text-sm text-black/45">Belum ada termin pembayaran.</p> : policies.map((item) => <div key={item.id} className="border border-black/10 bg-[#f5f3ee] p-3 text-sm"><div className="flex items-start justify-between gap-3"><div><div className="flex gap-3"><span className="font-semibold">{item.sequence_no}. {typeLabel(item.milestone_type)}</span><span>{paymentValue(item)}</span></div><p className="mt-1 text-black/55">Jatuh tempo: {dueLabel(item)}</p><p className="mt-1 text-black/45">Dapat dikembalikan: {item.refundable === true ? "Ya" : item.refundable === false ? "Tidak" : "Belum ditentukan"} · Dapat dinegosiasikan: {item.negotiable ? "Ya" : "Tidak"}</p>{item.cancellation_note ? <p className="mt-1 text-black/45">{item.cancellation_note}</p> : null}</div><button disabled={busy} onClick={() => deletePolicy(item)} className="shrink-0 border border-red-700 px-3 py-1.5 text-xs font-semibold text-red-700 disabled:opacity-40">Hapus</button></div></div>)}</div>
