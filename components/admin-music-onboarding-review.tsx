...
function actLabel(value?: string | null) {
  if(value==="original_artist")return "Artis dengan lagu original";
  if(value==="cover_performer"||value==="cover_entertainment")return "Penampil lagu cover";
  if(value==="mixed")return "Original + cover";
...
export function AdminMusicOnboardingReview({talentId}:{talentId:string}) {
  ...
  async function reclassify(){
    ...
    try{const res=await fetch("/api/internal-demo/admin/talent-onboarding-review",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({talentId,action:"reclassify_repertoire"})});const body=await res.json().catch(()=>null);if(!res.ok)throw new Error(body?.error??"Analisis AI gagal");setMessage("Daftar lagu berhasil dianalisis ulang. Periksa hasilnya sebelum menyetujui profil.");await refresh()}catch(e){setError(e instanceof Error?e.message:"Analisis AI gagal")}finally{setBusy(false)}
  }
  ...
  return <section className="mb-5 border border-black/10 bg-white p-5 md:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">Kurasi Musisi</p><h2 className="mt-2 text-xl font-semibold">Jenis penampilan & daftar lagu</h2></div><span className="border border-black/10 px-3 py-2 text-xs font-semibold">{actLabel(submission.act_type)}</span></div>
    <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
      {submission.act_type==="original_artist"?<div><b>Bersedia membawakan lagu cover</b><p>{yn(submission.willing_to_perform_covers)}</p></div>:null}
      {canCover?<div><b>Menerima permintaan lagu dari klien</b><p>{yn(submission.accepts_song_requests)}</p></div>:null}
    </div>
    {canCover?<>
      <div className="mt-5 border-t border-black/10 pt-4"><div className="flex items-center justify-between gap-3"><b className="text-sm">Contoh daftar lagu</b><span className={`text-xs font-semibold ${songs.length>=10&&songs.length<=20?"text-green-700":"text-red-700"}`}>{songs.length}/20 lagu</span></div>
      {songs.length?<ol className="mt-3 grid gap-1 text-sm text-black/65 sm:grid-cols-2">{songs.map((song,index)=><li key={`${song.title}-${song.artist}-${index}`}>{index+1}. {song.title||"—"} — {song.artist||"—"}</li>)}</ol>:<p className="mt-2 text-sm text-red-700">Belum ada daftar lagu.</p>}</div>
      <div className="mt-5 border border-black/10 bg-[#f8f7f3] p-4 text-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><b>Pengelompokan oleh AI</b><p className="mt-1 text-xs text-black/50">AI hanya membantu mengelompokkan berdasarkan Judul Lagu + Artis. Admin tetap memeriksa hasil sebelum profil disetujui.</p></div><span className={`text-xs font-semibold ${aiReady?"text-green-700":"text-amber-700"}`}>{aiReady?"Siap diperiksa":"Belum siap"}</span></div>
        <p className="mt-3"><b>Genre:</b> {(submission.repertoire_genres??[]).join(", ")||"—"}</p><p className="mt-1"><b>Gaya:</b> {(submission.repertoire_styles??[]).join(", ")||"—"}</p><p className="mt-1"><b>Era:</b> {(submission.repertoire_eras??[]).join(", ")||"—"}</p>
    ...
    </>:<p className="mt-4 text-sm text-black/55">Talent tidak menawarkan lagu cover, jadi contoh daftar lagu tidak diwajibkan.</p>}
