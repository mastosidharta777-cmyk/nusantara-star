...
export function AdminSupplyOnboardingLink({
  supplyId,
  supplyType,
}: {
  supplyId: string;
  ...
  async function createLink() {
    ...
    try {
      const response = await fetch("/api/internal-demo/admin/access-link", {
        ...
        body: JSON.stringify({ scope: "talent_onboarding", subjectId: supplyId }),
      });
  ...
  return (
    <section className="mt-5 border border-black/10 bg-white p-5 md:p-6">
      <p className="text-sm font-semibold">Secure {label} Onboarding</p>
      <p className="mt-1 text-xs text-black/45">Buat link 7 hari untuk profil/PIC. Link tidak membuka halaman admin atau data internal lain.</p>
