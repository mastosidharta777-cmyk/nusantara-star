...
function roleCanMutate(role: string, path: string) {
  ...
  if (role === "finance") {
    return ["/payment", "/payment-milestones", "/commercial-terms", "/deal-review", "/booking"].some((suffix) => path.includes(suffix));
  }
  if (role === "operations") {
    return ["/match-action", "/proposal-sent", "/booking", "/talent-commercial-profile"].some((suffix) => path.includes(suffix));
  }
  return false;
}
...
export async function middleware(request: NextRequest) {
  ...
  const roleResponse = await fetch(
    `${supabaseUrl}/rest/v1/admin_users?user_id=eq.${encodeURIComponent(user.id)}&active=eq.true&select=role&limit=1`,
  ...
  if (isAdminApi && MUTATION_METHODS.has(request.method) && !roleCanMutate(role, request.nextUrl.pathname)) {
...
export const config = {
  matcher: ["/admin/:path*", "/api/internal-demo/admin/:path*"],
};
