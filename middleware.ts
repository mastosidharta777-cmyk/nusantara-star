...
function roleCanMutate(role: string, path: string) {
  ...
  if (role === "finance") return ["/payment", "/payment-milestones", "/commercial-terms", "/deal-review", "/booking"].some((suffix) => path.includes(suffix));
  if (role === "operations") return ["/match-action", "/proposal-sent", "/booking", "/talent-commercial-profile", "/access-link"].some((suffix) => path.includes(suffix));
  return false;
}
